...
proc bearHMAC*(digestVtable: ptr HashClass; key, d: string): seq[byte] =
  ...
  hmacKeyInit(hKey, digestVtable, key.cstring, csize_t(key.len))
  hmacInit(hCtx, hKey, 0)
  hmacUpdate(hCtx, d.cstring, csize_t(d.len))
...
proc pemDecoderLoop(
  ...
  var inobj = false
  while length > 0:
    var tlen = pemDecoderPush(pemCtx, unsafeAddr pem[offset], csize_t(length)).int
    offset = offset + tlen
    length = length - tlen
...
proc decodeFromPem(skCtx: var SkeyDecoderContext, pem: string) =
  ...
  pemDecoderLoop(
    pem,
    cast[proc(ctx: pointer, pbytes: pointer, nbytes: csize_t) {.bearSslFunc.}](skeyDecoderPush),
    addr skCtx
  )
...
proc calcHash(alg: ptr HashClass, data: string, output: var array[64, byte]) =
  ...
  alg.init(pCtx)
  if data.len > 0:
    alg.update(pCtx, unsafeAddr data[0], csize_t(data.len))
...
proc bearSignRSPem*(data, key: string, alg: ptr HashClass, hashOid: cstring, hashLen: int): seq[byte] =
  ...
  assert(not s.isNil)
  if s(cast[ptr byte](hashOid), addr digest[0], csize_t(hashLen), addr pk, addr result[0]) != 1:
